        ...
        // } else if (state is HomePageSuccessStates) {
          return StreamBuilder(
            stream: context
                .read<HomePageCubit>()
                .getStreamOfList(),
            builder: (BuildContext context, AsyncSnapshot<List<NoteEntity>> snapshot) {
              if(snapshot.hasData && snapshot.data!.isNotEmpty){
                return ListView.builder(
                    itemBuilder: (context, index) {
                      final noteEntity=snapshot.data![index];
                      return  Container(
                        color: index==snapshot.data!.length-1?Colors.white:index.isOdd?Colors.purple.shade800:Colors.purple.shade200,
                        child: Container(
